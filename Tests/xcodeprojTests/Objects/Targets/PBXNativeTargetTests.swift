...
final class PBXNativeTargetTests: XCTestCase {
    ...
    func test_addDependency() throws {
        ...
        _ = target.addDependency(target: dependency)
        let targetDependency: PBXTargetDependency? = target.dependencyReferences.first?.materialize()

        XCTAssertEqual(targetDependency?.name, "Dependency")
        XCTAssertEqual(targetDependency?.targetReference, dependency.reference)
        let containerItemProxy: PBXContainerItemProxy? = targetDependency?.targetProxyReference?.materialize()
