...
extension PBXGroup: PlistSerializable {
    ...
    fileprivate func name(reference: String, proj: PBXProj) -> String? {
        if let group: PBXGroup = proj.objects.getReference(reference) {
            return group.name ?? group.path
        } else if let variantGroup: PBXVariantGroup = proj.objects.getReference(reference) {
            return variantGroup.name
        } else if let file: PBXFileReference = proj.objects.getReference(reference) {
